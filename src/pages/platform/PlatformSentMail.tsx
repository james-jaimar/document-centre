...
/**
 * Platform-level outgoing mail audit. Shows every email_outbox row with
 * tenant_id IS NULL — i.e. subscription receipts, plan changes, billing
 * alerts, platform admin invites, password resets for platform users, and
 * any platform_admin test sends. Tenant/branch mail is on /admin/sent-mail
 * and /branch/sent-mail respectively.
 */
export default function PlatformSentMail() {
  ...
  const load = async () => {
    ...
    const { data: accounts } = await supabase
      .from("email_accounts")
      .select("id,label,from_email,transport")
      .is("tenant_id", null)
      .is("branch_id", null);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      sentToday: rows.filter(
        (r) => r.status === "sent" && r.sent_at && new Date(r.sent_at) >= today,
      ).length,
  ...
  const retry = async (id: string) => {
    await supabase
      .from("email_outbox")
      .update({ status: "queued", attempts: 0, error_message: null, send_after: new Date().toISOString() })
      .eq("id", id);
    load();
  };
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Mail className="h-6 w-6" /> Platform Sent Mail
          </h1>
          <p className="text-muted-foreground">
            Every email sent from the platform mailbox — subscription receipts, trial
            notices, billing alerts, admin invites, password resets.
          </p>
        </div>
          ...
          {selected && (
            <>
              ...
              <div className="text-sm text-muted-foreground mt-2 space-y-1">
                ...
                <div>
                  <strong>Status:</strong> {selected.status} ({selected.attempts} attempts)
                </div>
                <div>
                  <strong>Queued:</strong> {new Date(selected.queued_at).toLocaleString()}
                </div>
                {selected.sent_at && (
                  <div>
                    <strong>Sent:</strong> {new Date(selected.sent_at).toLocaleString()}
                  </div>
                )}
                {selected.email_account_id && accountLabels[selected.email_account_id] && (
                  <div>
                    <strong>Sent via:</strong> {accountLabels[selected.email_account_id]}
                  </div>
                )}
              ...
              <div className="flex gap-2 mt-3">
                ...
                {(selected.status === "failed" || selected.status === "dlq") && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
