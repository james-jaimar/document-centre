...
export default function OrderFiles() {
  ...
  // If a session size lock exists, silently auto-apply the locked decision
  ...
  useEffect(() => {
    ...
    if (sessionSizeLock) {
      // Silent auto-apply path. Mark immediately so the effect can't re-fire.
  ...
  // ISO-size session-lock effect: for each cleanly-rendered ISO doc, either
  // (a) set the session lock if none exists yet, or (b) prompt the
  // locked-variant advisory if the doc's size doesn't match the lock.
  useEffect(() => {
    ...
    const candidate = documents.find((d) => {
      ...
      if (resolvedDocIds.current.has(d.id)) return false;
      // Skip docs that still need other advisories first
      const preflight = d.preflight_data as Record<string, any> | null;
      ...
      // Only consider docs with finalised dimensions
      const w = Number(d.page_width_mm);
      const h = Number(d.page_height_mm);
      if (!(w > 0 && h > 0)) return false;
      // Need a known ISO size to compare/lock against
    ...
    if (!sessionSizeLock) {
      // First ISO upload becomes the lock automatically.
    ...
    if (sizesMatch(w, h, sessionSizeLock.size.widthMm, sessionSizeLock.size.heightMm)) {
      // Already matches the lock — nothing to do.
      isoCheckedDocIds.current.add(candidate.id);
      return;
    }

    // Mismatched ISO upload — surface the locked-variant advisory.
    isoCheckedDocIds.current.add(candidate.id);
    setAdvisoryDoc({
      id: candidate.id,
