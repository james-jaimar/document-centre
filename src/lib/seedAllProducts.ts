...
export function seedRingBinders() {
  return seedFamily(
    "ring-binders",
    "Ring Binders",
    "Documents filed in D-Ring binders with a clear PVC front pocket. Optionally upload a separate single-sided Cover Sheet to slip into the front pocket — body pages are printed and hole-punched separately. The binder itself forms the back panel.",
    "BookOpen",
    2,
    [
      ...
      { name: "Paper Stock", option_type: "select", values: PAPER_ALL, is_required: true, sort_order: 2 },
      { name: "Print Colour", option_type: "select", values: PRINT_COLOUR, is_required: true, sort_order: 3 },
      { name: "Print Sides", option_type: "select", values: PRINT_SIDES, is_required: true, sort_order: 4 },
      { name: "Hole Punching", option_type: "select", values: HOLE_PUNCHING, is_required: false, sort_order: 5 },
      { name: "Tab Dividers", option_type: "select", values: TAB_DIVIDERS, is_required: false, sort_order: 6 },
      { name: "Inserts", option_type: "select", values: INSERTS, is_required: false, sort_order: 7 },
    ],
    PRICING_BOUND,
  );
}
