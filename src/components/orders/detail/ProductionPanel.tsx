  ...
  return (
    <div className="rounded-xl border bg-card overflow-hidden shadow-sm">
      ...
      <div className="p-5 space-y-5">
        ...
        {(artefacts?.assembly_report?.components?.length ?? 0) > 1 && (
          <div className="rounded-lg border-2 border-primary/30 bg-primary/5 px-3 py-2 space-y-2">
            <div className="text-[10px] font-bold uppercase tracking-wider text-primary/80">
              Production components
            </div>
            {artefacts!.assembly_report!.components!.map((c) => (
              <div key={c.storage_path} className="flex items-center justify-between gap-2">
                ...
                <Button
                  ...
                  disabled={openingPath === c.storage_path}
                  onClick={() => download(c.storage_path, c.label ?? c.component)}
                >
                  {openingPath === c.storage_path ? (
