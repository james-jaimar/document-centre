...
CREATE OR REPLACE FUNCTION public.set_branch_onboarding_step(
  _branch_id uuid,
  _step text,
  _done boolean
)
...
BEGIN
  ...
  -- Recompute completed_at based on required steps
  SELECT
...
GRANT EXECUTE ON FUNCTION public.set_branch_onboarding_step(uuid, text, boolean) TO authenticated;
