...
BEGIN
  ...
  -- Product links are authored against the metric catalogue. On an imperial
  -- branch, translate every size/paper/finishing code to its imperial twin so
  -- the branch keeps its product option lists.
  ...
  SELECT DISTINCT ON (m.product_family_id, m.catalog, COALESCE(m.sub_attribute,''), m.mapped_code)
    'branch', v_tenant, p_branch_id, m.product_family_id, m.catalog, m.sub_attribute, m.mapped_code, m.sort_order, m.is_default
  FROM (
    SELECT s.*,
      CASE
        WHEN s.catalog = 'print_attr' THEN s.item_code
        WHEN public.catalog_code_in_unit(s.catalog::text, s.item_code, s.sub_attribute, v_unit) THEN s.item_code
        ELSE COALESCE(public.catalog_unit_twin_code(s.catalog::text, s.item_code, v_unit), s.item_code)
      END AS mapped_code
    FROM (
  ...
  WHERE EXISTS (
    SELECT 1 FROM public.catalog_sizes cs WHERE m.catalog='size' AND cs.scope_type='branch' AND cs.branch_id=p_branch_id AND cs.code=m.mapped_code
    UNION ALL SELECT 1 FROM public.catalog_papers cp WHERE m.catalog='paper' AND cp.scope_type='branch' AND cp.branch_id=p_branch_id AND cp.code=m.mapped_code
    UNION ALL SELECT 1 FROM public.catalog_finishing cf WHERE m.catalog='finishing' AND cf.scope_type='branch' AND cf.branch_id=p_branch_id AND cf.code=m.mapped_code
    UNION ALL SELECT 1 WHERE m.catalog='print_attr'
  )
  AND NOT EXISTS (SELECT 1 FROM public.product_catalog_links b
    WHERE b.scope_type='branch' AND b.branch_id=p_branch_id
      AND b.product_family_id=m.product_family_id AND b.catalog=m.catalog
      AND COALESCE(b.sub_attribute,'') = COALESCE(m.sub_attribute,'') AND b.item_code=m.mapped_code);
END;
$function$;