...
-- 1. Rebuild the branch unique index to include variant_code (matches master/tenant).
...
-- 2. Sync master → tenant must carry variant_code and match on it.
...
AS $function$
BEGIN
  -- ===== CLICKS (key: size, colour, sides, variant_code) =====
  INSERT INTO public.rate_card_clicks
    (scope_type, tenant_id, size, colour, sides, variant_code, sell_price, cost_price, is_active)
  SELECT 'tenant', p_tenant_id, m.size, m.colour, m.sides, m.variant_code, m.sell_price, m.cost_price, m.is_active
  ...
  WHERE t.scope_type = 'tenant' AND t.tenant_id = p_tenant_id
    ...
    AND COALESCE(t.variant_code,'') = COALESCE(m.variant_code,'');
  ...
  WHERE t.scope_type = 'tenant' AND t.tenant_id = p_tenant_id
    AND NOT EXISTS (
      ...
      WHERE m.scope_type = 'master'
        ...
        AND COALESCE(m.variant_code,'') = COALESCE(t.variant_code,'')
    );
...
-- 3. Clone master → tenant: carry variant_code and match on it.
...
-- 4. Clone tenant → branch: carry variant_code and match on it.
...
BEGIN
  ...
  INSERT INTO public.rate_card_clicks
    (scope_type, tenant_id, branch_id, size, colour, sides, variant_code, sell_price, cost_price, is_active)
  SELECT 'branch'::rate_card_scope, v_tenant, p_branch_id, t.size, t.colour, t.sides, t.variant_code, t.sell_price, t.cost_price, t.is_active
  ...
  WHERE t.scope_type = 'tenant'::rate_card_scope AND t.tenant_id = v_tenant
    AND NOT EXISTS (
      ...
      WHERE b.scope_type = 'branch'::rate_card_scope AND b.branch_id = p_branch_id
        ...
        AND COALESCE(b.variant_code,'') = COALESCE(t.variant_code,'')
    );
