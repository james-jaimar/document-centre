...
BEGIN
  ...
  INSERT INTO public.orders (
    user_id, tenant_id, app_id, branch_id, order_status, total_price, currency,
    subtotal, total_amount, amount_due, ordered_by_profile_id, customer_email,
    customer_name, created_by_admin_profile_id, source_channel, metadata
  ) VALUES (
    v_customer_profile_id, p_tenant_id, p_app_id, p_branch_id,
    'quoted'::public.order_status, COALESCE(p_total_amount, 0),
    COALESCE(NULLIF(p_currency, ''), 'ZAR'),
    COALESCE(p_total_amount, 0), COALESCE(p_total_amount, 0), COALESCE(p_total_amount, 0),
    v_customer_profile_id, v_clean_email,
    NULLIF(trim(COALESCE(p_customer_name, '')), ''),
    v_actor_id, 'spec_quote',
    jsonb_build_object('is_spec_quote_holding', true)
  )
  RETURNING orders.id INTO v_holding_order_id;

  INSERT INTO public.order_items (
    order_id, product_family_id, quantity, unit_price, build_status, spec, title
  ) VALUES (
    v_holding_order_id, p_product_family_id, p_quantity, COALESCE(p_unit_price, 0),
    'ready'::public.build_status, COALESCE(p_spec, '{}'::jsonb),
  ...
  INSERT INTO public.quotes (
    app_id, tenant_id, branch_id, quote_number, name, customer_profile_id,
    customer_email, customer_name, created_by_profile_id, created_via,
    source_order_id, quote_status, valid_until, currency, subtotal,
    total_amount, notes_internal, metadata
  ) VALUES (
    p_app_id, p_tenant_id, p_branch_id, v_quote_number,
    NULLIF(trim(COALESCE(p_quote_name, '')), ''), v_customer_profile_id,
    v_clean_email, NULLIF(trim(COALESCE(p_customer_name, '')), ''),
    v_actor_id, v_created_via, v_holding_order_id,
    'active'::public.quote_status, v_valid_until,
    COALESCE(NULLIF(p_currency, ''), 'ZAR'),
    COALESCE(p_total_amount, 0), COALESCE(p_total_amount, 0),
  ...
  INSERT INTO public.quote_items (
    quote_id, sequence_no, product_family_id, product_name, job_name,
    quantity, unit_price, net_price, gross_price, source_job_id,
    product_snapshot, configuration
  ) VALUES (
    v_quote_id, 1, p_product_family_id,
