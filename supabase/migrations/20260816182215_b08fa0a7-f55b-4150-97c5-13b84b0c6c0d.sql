CREATE OR REPLACE FUNCTION public.catalog_code_in_unit(p_catalog text, p_code text, p_sub_attribute text, p_unit text)
RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
  IF p_catalog = 'size' THEN
    SELECT EXISTS(SELECT 1 FROM public.catalog_sizes WHERE scope_type='master' AND code=p_code AND unit_system=p_unit) INTO v_ok;
  ELSIF p_catalog = 'paper' THEN
    SELECT EXISTS(SELECT 1 FROM public.catalog_papers WHERE scope_type='master' AND code=p_code AND unit_system=p_unit) INTO v_ok;
  ELSIF p_catalog = 'finishing' THEN
    SELECT EXISTS(SELECT 1 FROM public.catalog_finishing WHERE scope_type='master' AND code=p_code AND unit_system=p_unit) INTO v_ok;
  ELSE
    v_ok := true; -- print attributes are unit agnostic
  END IF;
  RETURN v_ok;
END;
$function$;
