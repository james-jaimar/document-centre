...
-- Extend branch-creation trigger to also seed branch_capabilities
...
-- Extend ensure_branch_pricing_seeded to also seed capabilities (safety-net)
...
BEGIN
  ...
  -- If pricing not yet seeded, do the full clone
  ...
  -- Pricing already seeded — but check capabilities as a self-heal
...
-- Backfill: seed capabilities for every branch that has none
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    ...
    WHERE NOT EXISTS (SELECT 1 FROM public.branch_capabilities bc WHERE bc.branch_id = b.id)
  LOOP
    BEGIN
      PERFORM public.seed_branch_capabilities(r.id);
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING 'backfill capability seed failed for branch %: %', r.id, SQLERRM;
    END;
  END LOOP;
END $$;
