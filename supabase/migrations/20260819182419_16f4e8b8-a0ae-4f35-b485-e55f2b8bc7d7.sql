  ...
  FROM (
    SELECT s.*,
      CASE
        ...
        WHEN public.catalog_code_in_unit(s.catalog, s.item_code, s.sub_attribute, v_unit) THEN s.item_code
        ELSE COALESCE(public.catalog_unit_twin_code(s.catalog, s.item_code, v_unit), s.item_code)
      END AS mapped_code
    FROM (
