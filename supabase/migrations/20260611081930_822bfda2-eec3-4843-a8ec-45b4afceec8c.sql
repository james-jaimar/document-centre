...
AS $function$
BEGIN
  -- Hard stop the old VPS/Celery email worker that is still polling with stale SMTP-only code.
  -- It was observed claiming rows as pdf-server:srv1516161:<pid> and failing graph_oauth accounts.
  IF p_worker_id LIKE 'pdf-server:srv1516161:%' THEN
    RETURN;
  END IF;
